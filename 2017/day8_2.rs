...
fn main () {
	...
	let mut max_val_evahh = <i32>::min_value();
	let mut max_key_evahh = String::from("");
	for condition in conditions.iter() {
		...
		for elem in registers.iter() {
			if elem.1 > &max_val_evahh {
				max_val_evahh = *elem.1;
				max_key_evahh = elem.0.to_string();
			}
		}
	}
	...
	println!("Ever:  {:?} {:?}", max_key_evahh, max_val_evahh);
}